...
package net.tongsuo.sun.security.util;

import net.tongsuo.sun.security.action.GetPropertyAction;
import net.tongsuo.sun.security.x509.CertificateExtensions;
 ...
 *
 * If the system property "net.tongsuo.ssl.debug" is not defined, the debug logging
 * is turned off.  If the system property "net.tongsuo.ssl.debug" is defined as
 * empty, the debug logger is specified by System.getLogger("net.tongsuo.ssl"),
 * and applications can customize and configure the logger or use external
 * logging mechanisms.  If the system property "net.tongsuo.ssl.debug" is defined
...
public class SSLLogger {
    ...
    static {
        String p = GetPropertyAction.privilegedGetProperty("net.tongsuo.ssl.debug");
        if (p != null) {
            if (p.isEmpty()) {
                property = "";
                logger = Logger.getLogger("net.tongsuo.ssl");
            } else {
                ...
                logger = new SSLConsoleLogger("net.tongsuo.ssl", p);
            }
            isOn = true;
        } else {
            property = null;
    ...
    /**
     * Return true if the "net.tongsuo.ssl.debug" property contains the
